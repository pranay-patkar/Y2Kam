...
## 1998 A retro digicam photo editor, styled like it's
...
```
Photo Adjustments RGB Balance LUT
grade Overlays & stamps Frame (+
caption) Win95 wrap Export
```
...
| Stage | What happens |
| --- | --- |
|   | Source image drawn at target |
| 1. Base draw | size; optional pixelation via a |
|   | downscale/upscale trick |
|   | Red and blue channels sampled |
| 2. Chromatic | with a pixel offset, green stays |
| aberration | put — real channel-split fringing |
|   | Saturation, grain, fade/contrast, |
| 3. Tone | all applied in one pass over the |
| adjustments | ImageData buffer |


| Stage | What happens |
| --- | --- |
|   | One of 22 hand-written channel- |
| 4. LUT color | mix functions re-maps every |
| grade | pixel's RGB |
|   | Independent Red/Green/Blue |
|   | channel offsets, applied last so |
| 5. RGB Balance | they fine-tune whatever the LUT |
|   | produced |
| 6. Flash / light | Radial and linear gradients, |
| leak / vignette / | screen -blended light leaks, |
| scan lines | scaled scan-line density |
|   | Date stamps, Play/battery icons, |
| 7. Stamps & | custom text, and full-frame |
| overlays | overlays (BSOD, VHS static, REC |
|   | indicator, low battery, starfield) |
|   | Wraps the canvas with padding |
| 8. Frame | — 8 styles, from Polaroid to |
|   | Ticket Stub (see below) |
|   | (optional, export-time) Wraps |
|   | everything in a full fake OS |
| 9. Win95 | window with a customizable |
| window wrap | titlebar color and a working |
|   | clock |
...
style.css all styling, incl. the
4 theme variants
script.js filters, LUTs,
overlays, stamps, frames, export logic
README.md
```
...
username>/y2kam.git
cd y2kam
# just open index.html — or, for a local
dev server:
